...
int getQuantiy(string idClass, listStudent &ls) {
	
	int quantity = 0;
	ptrStudent current = ls.head;
	
	if (current == NULL) return -1;
	
	
	while(current != NULL && idClass <= current->value.idClass) {
		if (idClass == current->value.idClass) {
			quantity += 1;
		}
