...
void mouseHighlightUpdate2(int &on, int &selected){
	setcolor(BLACK);
...
void createBottom(){
	...
	setcolor(BLACK);
	outtextxy(200, 610, "ESC             : Thoat cua so/ Thoat nhap.");
	outtextxy(200, 630, "ENTER           : Chon muc/ Hoan thanh nhap."); 
	outtextxy(200, 650, "UP/DOWN         : Di chuyen len/ xuong.");
	outtextxy(200, 670, "CLICK           : Chon muc.");
...
void drawDisplaySearchBox(){
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	
	outtextxy(ClASSPOINTX - 140, CLASSPOINTY + 250, "Tim kiem: ");
	setcolor(LIGHTGRAY);
	rectangle(ClASSPOINTX, CLASSPOINTY + 240, CLASLPOINTX, CLASLPOINTY + 270);
	
	setDefault();	
}

void clearDisplaySearchBox(){
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	setbkcolor(WHITE);
	
	bar(ClASSPOINTX - 150, CLASSPOINTY + 240, CLASLPOINTX + 1, CLASLPOINTY + 271);
	
	setDefault();
}

void highlightDisplaySearchBox(int COLOR){
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	setcolor(COLOR);
...
void clearTextInSearchBox(){
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	setbkcolor(WHITE);
	
	bar(ClASSPOINTX + 1, CLASSPOINTY + 241, CLASLPOINTX - 1, CLASLPOINTY + 269);
	
	setDefault();
}

void outtextInSearchBox(string text) {
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	outtextxy(ClASSPOINTX + 20, CLASSPOINTY + 245, tochar(text));
}


void outtextErrorSearchBox(string text, int COLOR) {
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	setcolor(COLOR);
	outtextxy(ClASSPOINTX - 140, CLASSPOINTY + 280, tochar(text));
	delay(1000);
	
	setbkcolor(WHITE);
	bar(ClASSPOINTX - 145, CLASSPOINTY + 275, CLASLPOINTX + 30, CLASSPOINTY + 305);
	
	setDefault();
}

void clearLeftBarSearchClass() {
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	setbkcolor(WHITE);
	
	bar(ClASSPOINTX - 150, CLASSPOINTY - 27, CLASLPOINTX + 1, CLASLPOINTY + 271);
	
	setDefault();
	
}

void displayTitleClass(string idClass, int quantity) {
	int ClASSPOINTX = MENUSPOINTX + 150, CLASLPOINTX = MENUSPOINTX + 290;
	int CLASSPOINTY = MENUSPOINTY + 155, CLASLPOINTY = MENUSPOINTY + 155;
	
	
	setfillstyle(SOLID_FILL, LIGHTBLUE);
	bar(ClASSPOINTX - 140, CLASSPOINTY + 45, CLASLPOINTX, CLASSPOINTY + 110);
	
	setcolor(WHITE);
	setbkcolor(LIGHTBLUE);
	outtextxy(ClASSPOINTX - 125, CLASSPOINTY + 50, tochar("DANH SACH LOP " + idClass));
	outtextxy(ClASSPOINTX - 70, CLASSPOINTY + 80, tochar("SO LUONG " + to_string(quantity)));
...
void drawPrintRegisteredStudentListV2(ptrClassForSubject cfs){
	...
	// tao duong ke ngnang
	if(cfs != NULL && cfs->studentMin <= cfs->lr->size){
		...
		setbkcolor(WHITE);
		setcolor(LIGHTGRAY);
		outtextxy(X + 330, TEXTLPOINTY1 + 5, "Chua du dien kien mo lop!");
	}
	else {
		setcolor(LIGHTGRAY);
		line(X + 50, TEXTLPOINTY1 - 30, X + 50, TEXTLPOINTY1);
