...
bool validFormatIdClass(string idClass){
	if (idClass.size() != 9 || idClass[0] != 'D') return false;
	
	return isdigit(idClass[1]) && isdigit(idClass[2]) && isdigit(idClass[7]) && isdigit(idClass[8]) &&
			isalpha(idClass[3]) && isalpha(idClass[4]) && isalpha(idClass[5]) && isalpha(idClass[6]); 
}

bool isExistIdClass(string idClass, listClass &lc){
	for (int i = 0; i < lc.size; i++) {
		if(lc.idClass[i] == idClass){
			return true;
		}
	}
	return false;
...
void displayStudentWithClass(listStudent &ls, listClass &lc){   // xem danh sach theo lop 
	...
	while(run1){
		if(kbhit()){
			...
			switch(ascii){
				case ET: 
					...
					clearLeftBarSearchClass();
					displayTitleClass(lc.idClass[currentClass], getSizeClass(ls, lc.idClass[currentClass]));
						...
						while(run2){
							if(kbhit()){
								...
								if(ascii == ESC){
									...
									clearLeftBarSearchClass();
									drawDisplaySearchBox();
									highlightDisplaySearchBox(LIGHTGRAY);
									outtextInSearchBox(idClass);
		...
		if(ismouseclick(WM_LBUTTONDOWN)){ 
			...
			getmouseclick(WM_LBUTTONDOWN, x, y);
			clearmouseclick(WM_LBUTTONDOWN);
			if(clickInRange(x, y, ClASSPOINTX, CLASSPOINTY + 240, CLASLPOINTX, CLASLPOINTY + 280)){
			
				highlightDisplaySearchBox(LIGHTBLUE);
				...
				while(true){
					if(kbhit()){
						...
						if(ascii == ET){
							if (validFormatIdClass(idClass)) {
								if(isExistIdClass(idClass, lc)) {
									
									clearLeftBarSearchClass();
									displayTitleClass(idClass, getSizeClass(ls, idClass));
									
									firstStudent = 0; 
								
									createListStudentWithClass(); // ve bang liet ke thong tin sinh vien theo lop
						
									if(firstStudent < getSizeClass(ls, idClass)){ // so luong sinh vien cua mot lop
									
										findListStudentWithClass(ls, firstStudent, idClass); // tim thong tin sinh vien trong danh sach lop va in ra man hinh
										createScrollBar(firstStudent, getSizeClass(ls, idClass));
											...
											if(kbhit()){
												...
												if(ascii == ESC){
													...
													if(ceil((float)firstStudent / 12) > 1){ 
														...
														if(firstStudent >= 0){
															findListStudentWithClass(ls, firstStudent, idClass); 
															createScrollBar(firstStudent, getSizeClass(ls, idClass));
														}
													}
												}
												else if(ascii == DOWN){
													if(firstStudent < getSizeClass(ls, idClass)){
														findListStudentWithClass(ls, firstStudent, idClass); 
														createScrollBar(firstStudent, getSizeClass(ls, idClass));
													}
												}
									...
									outtextErrorSearchBox("Lop khong ton tai!", LIGHTRED);
								}
							}
							else {
								outtextErrorSearchBox("Ma lop khong dung dinh dang!", LIGHTRED);
							}
						}
